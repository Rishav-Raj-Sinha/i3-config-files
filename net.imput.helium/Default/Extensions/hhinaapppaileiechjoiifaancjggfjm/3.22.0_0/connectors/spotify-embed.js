"use strict";
(() => {
  // src/connectors/spotify-embed.ts
  Connector.playerSelector = "[data-testid=embed-widget-container]";
  Connector.pauseButtonSelector = "[class^=PlayButton_buttonWrapper___][aria-label=Pause]";
  Connector.getArtist = () => {
  ...
  Connector.isTrackArtDefault = () => Boolean(
    Util.getAttrFromSelectors(
      "[class*=_spotifyLogoContainer__] a",
      "href"
    )?.match(/\/artist|playlist\//)
  );
  Connector.isPodcast = () => Boolean(
  ...
  Connector.durationSelector = "li[class*=TracklistRow_isCurrentTrack__] [class^=TracklistRow_durationCell__]";
